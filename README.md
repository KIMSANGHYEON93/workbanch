...
##
<img width="1440" height="1000" alt="image" src="https://github.com/user-attachments/assets/e5221bc6-55fd-4ed2-b16c-d3c50782e24f" />
<img width="1440" height="1000" alt="image" src="https://github.com/user-attachments/assets/f3d6c01c-0ee3-48b5-b7d8-0e5631031fd9" />
<img width="1440" height="1000" alt="image" src="https://github.com/user-attachments/assets/d90c7500-12ce-4dd4-8426-e6c136ca2030" />
<img width="1440" height="1000" alt="image" src="https://github.com/user-attachments/assets/85556efc-3e36-4fcb-973f-18c34f2182bc" />
<img width="1440" height="1000" alt="image" src="https://github.com/user-attachments/assets/547adcb4-61df-4828-962a-38982de35183" />
<img width="1440" height="1000" alt="image" src="https://github.com/user-attachments/assets/103c5e83-355e-436d-bb66-24b1931b9f01" />
...
이슈 상세와 페이지 보기에 `AttachmentPanel` 을 얹는다. 내려받기는 Blazor 컴포넌트가 아니라
`GET /attachments/{id}` 엔드포인트가 처리한다 — 바이트를 회로(circuit)로 흘려보내지 않기 위함.

| 제공자 | 용도 |
|---|---|
| `AzureBlob` (기본값) | 운영 |
| `Local` | 개발 — Azurite·실 계정 없이 업로드 경로를 실제로 태울 수 있다 |

### ★ 보안 결정 3개

1. **저장 경로에 사용자 파일명을 쓰지 않는다.** `../../web.config` 같은 이름이 그대로 경로가 되면
   저장소 밖으로 나간다. 경로는 `{GUID}{안전한 확장자}` 이고, 원본 이름은 DB 컬럼에만 남아
   내려줄 때 쓰인다. 확장자는 영숫자만 허용하고 애매하면 통째로 버린다.
2. **내려받기는 항상 `Content-Disposition: attachment`.** 그렇지 않으면 업로드된 `.html`·`.svg` 가
   **이 앱의 출처에서** 렌더돼 저장형 XSS 가 된다 — 세션 쿠키가 그대로 딸려 있는 출처다.
   Blob 업로드 시에도 같은 헤더를 못박고, 앱 전체에 `X-Content-Type-Options: nosniff` 를 건다.
3. **로컬 저장소는 루트 밖 경로를 읽지도 지우지도 않는다.** 경로는 우리가 만들지만, DB 값이 어떤
   경위로든 바뀌었을 때 그 값으로 임의 파일을 건드리는 일이 없어야 한다.

### 실패했을 때 무엇이 남는가

| 상황 | 처리 |
|---|---|
| 메타데이터 저장 실패 | 방금 올린 바이트를 지운다 — 아무도 찾을 수 없는 파일이 쌓이지 않는다 |
| 삭제 시 바이트 제거 실패 | DB 행을 **먼저** 지웠으므로 목록에는 남지 않는다. 고아 blob 은 조용하고 무해하다 |
| 부모(이슈·페이지) 삭제 | 행은 `CASCADE` 로 사라지고 **바이트는 저장소에 남는다**(알려진 성질, 정리는 별도 작업) |
| DB·저장소 장애 시 다운로드 | `503` + 조치 문구. 처리되지 않은 예외로 끝내지 않는다(개발 환경에서는 스택이 응답에 실린다) |

### 운영 기동 가드

- `Provider=Local` 인데 Development 가 아니면 **기동을 막는다.** 로컬 디스크는 재배포·스케일아웃에서
  첨부가 사라진다. 단일 서버에 보관할 의도라면 `AllowLocalOutsideDevelopment=true` 로 **명시**해야 한다.
- `Provider=AzureBlob` 인데 접속 문자열이 없으면 기동을 막는다.

세 경우 모두 실측했다 — ①·② 차단, ③ 명시 허용 시 기동.

---

## 칸반 보드 (11단계)

`/board` — 상태별 6열, 프로젝트·담당자 필터. 목록은 완료를 감추지만 **보드는 완료·취소 열을
가지므로 닫힌 이슈까지 가져온다**(안 그러면 그 열이 영원히 비어 보인다).

- **끌어놓기는 JS interop 0줄** — Blazor 의 드래그 이벤트만 쓴다.
- **끌어놓기를 못 쓰는 사람을 위한 경로가 있다** — 카드마다 ◀ ▶ 버튼(키보드 조작 가능,
  양 끝 열에서는 비활성). 마우스만 되는 화면은 접근성 이전에 그냥 못 쓰는 화면이다.
- 열 순서는 `Enum.GetValues` 에 맡기지 않고 `BoardLayout` 에서 못박는다 — enum 값 순서는
  저장 형식이고 보드 순서는 업무 흐름이다. 둘이 같아 보여도 이유가 다르다.

---

## 검색 (12단계)

`/search?q=…` + 모든 화면 상단의 검색창. 이슈(키·제목) · 페이지(제목·본문) · 프로젝트(키·이름·설명)를
한 번에 찾아 **종류별로 묶어** 보여준다.

### 규칙 3가지

1. **빈 검색어가 전체를 쏟아내지 않는다.** 2자 미만은 아예 조회하지 않는다 — 한 글자는 사실상
   전수 조회라, 결과가 상한에서 잘려 도움이 되지 않으면서 DB 만 훑는다.
2. **검색은 완료·취소된 이슈도 찾는다.** 목록의 "완료 숨김" 을 물려받으면 안 된다 —
   끝낸 일을 다시 찾는 것이 검색의 주된 쓰임이다.
3. **잘렸으면 잘렸다고 말한다.** 종류별 `SearchLimits.MaxHitsPerKind` 건까지 표시하고,
   넘으면 화면이 그 사실을 알린다. 조용히 자르면 사용자는 **없는 것과 구분할 수 없다.**

경계값은 `SearchLimits` 한 곳에 있다 — 화면이 다른 상한을 안내하면 사용자는 "왜 안 찾아지지" 를
스스로 알아낼 수 없다.
...
2단계에서 스키마만 만들어 두고 쓰이지 않던 `ProjectMember` / `ProjectRole` 을 실제 판정에 연결했다.

| 역할 | 할 수 있는 것 |
|---|---|
| (로그인) | **보기** — 모든 프로젝트·이슈·페이지 |
| `Viewer` 보기 | 읽기만 |
| `Member` 구성원 | 그 프로젝트의 이슈·페이지 변경 |
| `Admin` 관리자 | + 프로젝트 설정·삭제, 구성원 관리 |

화면: `/projects/{id}/members`

### 규칙 4가지

1. **구성원이 없는 프로젝트는 "열린 프로젝트"** — 로그인한 누구나 쓸 수 있다.
   이 규칙이 없으면 권한 기능을 붙이는 순간 **기존 프로젝트가 전부 손댈 수 없게 된다.**
   화면이 그 상태를 배지와 안내문으로 말한다.
2. **새 프로젝트는 만든 사람을 관리자로 함께 등록한다** — 열린 상태로 태어나지 않고 책임자가 분명해진다.
3. **마지막 관리자는 내리거나 뺄 수 없다.** 구성원이 남아 있는 한 열린 프로젝트로 돌아가지 않으므로,
   내리는 순간 **아무도 그 프로젝트를 다시 관리할 수 없는 영구 잠김**이 된다.
4. **활성 계정 확인은 새로 추가할 때만 한다** — 역할 변경까지 막으면, 계정이 비활성화된 순간
   관리자가 그 사람을 내리지도 빼지도 못한다. *(이 결함은 테스트를 쓰다가 드러났다.)*

### 판정은 서비스가 한다

화면에서 버튼을 감추는 것은 **안내이지 통제가 아니다.** `IssueService` · `PageService` ·
`ProjectService` 가 쓰기마다 권한을 다시 확인하고, 그 강제 여부를 화면을 거치지 않고
서비스를 직접 불러 검증한다(`PermissionEnforcementTests`).

페이지를 다른 프로젝트로 옮길 때는 **양쪽 모두**에 권한이 있어야 한다 — 한쪽만 보면
권한 없는 프로젝트로 문서를 밀어 넣거나 빼낼 수 있다.
...
| 컴포넌트 | **bUnit** (실제 서비스 + 메모리 저장소) | 화면이 실제로 그려지는가, 클릭·드롭이 동작하는가 |
...
- **보드 외 화면의 정상 경로** — bUnit 은 11단계에서 보드에만 적용했다. 목록·상세·에디터는
  라우팅·DI·오류 경로만 실기동으로 확인돼 있다(모든 경로 HTTP 200 · 미처리 예외 0).
  같은 방식으로 넓히면 된다.
